...
//	}
	...
	public SecurityFilterChain filterChain(HttpSecurity http) {
		return http.authorizeRequests()
				...
				.defaultSuccessUrl("/index") // 登录成功后自动跳转的地址(未必, 如果登录之前的地址是 /from, 那么登录之后会跳转到 from) , 当然如果你把 defaultSuccessUrl 的第二个参数设置为 true, 那么效果和 successForwardUrl 一样, 不管登录之前是哪个网址, 登录之后都是配置的 /index 网址
//				.successForwardUrl("/index") // 不管登录之前是在哪个地址,登录之后都跳转到 /index , 相当于 defaultSuccessUrl("/index", true)
//				.failureForwardUrl("/error") // 登录失败转发的地址
